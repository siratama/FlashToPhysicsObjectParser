...
class Main {
	...
	private function outputCircle(){
		...
		var physicsObject:PhysicsObject;
		physicsObject = flashToPhysicsObjectParser.getPhysicsObject(sampleCircle, sampleCircle.ball);
		trace(physicsObject);
	...
	private function outputPolygon(){
		...
		polygon = cast flashToPhysicsObjectParser.getPhysicsObject(samplePolygon, samplePolygon.flipper);
		trace(polygon);
